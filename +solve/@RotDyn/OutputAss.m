...
if or(~isempty(obj.input.Bearing),~isempty(obj.input.TorBearing))
    ...
    if ~isempty(obj.input.LUTBearing)
        for i=1:size(obj.input.LUTBearing,1)
            ...
            x=Ass.Part{obj.output.TotalElement,1}.mesh.nodes(obj.input.LUTBearing(i,1),1);
            y=Ass.Part{obj.output.TotalElement,1}.mesh.nodes(obj.input.LUTBearing(i,1),2);
            z=Ass.Part{obj.output.TotalElement,1}.mesh.nodes(obj.input.LUTBearing(i,1),3);
            Ass=AddCnode(Ass,x,y,z);
            Acc_Cnode=GetNCnode(Ass);
            Ass=AddMaster(Ass,obj.output.TotalElement,obj.input.LUTBearing(i,1));
            Ass=AddMaster(Ass,0,Acc_Cnode);
            Acc_Mas=GetNMaster(Ass);
            Ass=SetLUTBearing(Ass,Acc_Mas-1,Acc_Mas,i);
            Ass=SetCnode(Ass,Acc_Cnode,Acc_ET);
            Ass=AddBoundary(Ass,0,'No',Acc_Cnode);
            AccBC=GetNBoundary(Ass);
            Ass=SetBoundaryType(Ass,AccBC,Bound);
