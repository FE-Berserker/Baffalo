function Bearing = ConvertHousingBearing(inputBearing,NumNode)
% Convert HousingBearing to AMRotor bearng matrix
% Author : Xie Yu

Num=size(inputBearing,1);
Bearing=cell(1,Num);
for i=1:Num
    Bearing{1,i}.Name=strcat('HousingBearing',num2str(i));
    Bearing{1,i}.Type='HousingBearing';
    Bearing{1,i}.Node=inputBearing(i,1)+NumNode;
...
D = sparse(6,6);
D(1,1)=Damping(2)*1000;
D(2,2)=Damping(3)*1000;
D(1,2)=Damping(4)*1000;
D(2,1)=Damping(5)*1000;
D(3,3)=Damping(1)*1000;
damping_matrix = D;
end