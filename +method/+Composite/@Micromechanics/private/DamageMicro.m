function [props, MoS] = DamageMicro(obj,props, MoS, CurrentPly)
% +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
% --------------------------
             ...
             if (minMoS < 0) && (MoS.Micro(b,g).MinMoS - minMoS - tol) < 0 || AlreadyFailed
                if ~AlreadyFailed
                    ...
                    if CurrentPly > 0
                        ...
                        text = ['      Failure in subcell ',num2str(b),',',num2str(g), ' . '];
                    end
                    disp(text);
                    % if OutInfo.Format == "txt"
                    %     WriteText(OutInfo.Format, fid, Fmt, ActXWord, text, 'Normal', [0,1])
                ...
                switch(props.RUC.matsCh(b,g))
                    case {'F', '1'}
                        props.RUC.mats(b,g) = props.FiberDam.ID;
                        props.RUC.matsCh(b,g) = '1';
                    case {'M', '2'}
                        props.RUC.mats(b,g) = props.MatrixDam.ID;
                        props.RUC.matsCh(b,g) = '2';
                    case {'I', '3'}
                        props.RUC.mats(b,g) = props.InterfaceDam.ID;
    ...
    if MoS.NewFailure
        ...
        [props]=RunMicro(obj.params.Theory,...
            obj.params.Name,...
            Vol,...
            Constits,props.RUC,props);
