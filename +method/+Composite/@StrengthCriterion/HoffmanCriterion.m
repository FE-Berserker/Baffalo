function obj=HoffmanCriterion(obj)
%函数功能：基于Hoffman强度准则计算强度比。
%调用格式：HoffmanCriterion(Sigma,Xt,Xc,Yt,Yc,S)。
%输入参数：Sigma—为正轴应力列向量；
...
F55=1./F5.^2;
F66=1./F6.^2;
F12=1./(F1t.*F1c)+1./(F2t.*F2c)-1./(F3t.*F3c);
F13=1./(F1t.*F1c)-1./(F2t.*F2c)+1./(F3t.*F3c);
F23=-1./(F1t.*F1c)+1./(F2t.*F2c)+1./(F3t.*F3c);
