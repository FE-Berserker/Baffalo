classdef Rplot <matlab.mixin.Copyable
    %RPLOT is a class to plot figure for ROTA
    %Yu Xie 2023
    ...
    properties
        ...
        %store variables used when making multiple Rplot plots in the same window:
        multi=struct('orig',[0 0],...   origin (x,y) of the current Rplot plot in normalized
            'size',[1 1],...   size (w,h) of the current Rplot plot in normalized values
            'radar',0,...     
            'active',false)

        %Stores variables relative to Rplot updating
        ...
        axe_options %Structure holding axe options

        annotation_options %Draw annotation options

        arrow_options %Draw arrow options
    ...
    methods
        function obj=Rplot(varargin)
            % Rplot Constructor for the class
            %
            % Example syntax (default arguments): Rplot_object=Rplot('x',x_variable,'y',y_variable,'color',color_variable)
            % This function is used to create a Rplot object, and provide
            ...
            set_axe_options(obj);
            set_annotation_options(obj);
            set_arrow_options(obj);
        end
