...
%
% syntax: Rplot_object.draw()
% Call draw on an array of Rplot objects in order to have
% multiple Rplots on a single figure.
% An optional logical argument can be given. Use
% Rplot_object.draw(false) in order to allow the nex Rplot plot to be
...
%% Handle call of draw() on array of Rplot objects by dividing the figure
...
if numel(obj)>1
    ...
    if isempty(obj(1).bigtitle)
        ...
        tmp=text(0,0,obj(1).bigtitle,...
            'FontWeight','bold',...
            'Interpreter',obj(1).text_options.titleinterpreter,...
...
%% If x init vertices is empty the object probably is so we skip (happens for multiple graphs)
if isempty(obj.aes.x)&&isempty(obj.aes.init)&&isempty(obj.aes.vertices)
    return
end
...
%Handles the multiple Rplot case: we set up subtightplot so
...
mysubtightplot=@(m,n,p,gap,marg_h,marg_w)my_tightplot(m,n,p,...
    ...
    obj.multi.radar,'Parent',obj.parent);
...
if iscell(temp_aes.color) && ~iscellstr(temp_aes.color)
    ...
    if isnan(obj.continuous_color_options.active)
        if length(uni_color)>15 && ~iscellstr(uni_color) %#ok<ISCLSTR> 
            obj.continuous_color_options.active = true;
        else
            obj.continuous_color_options.active = false;
        end
    elseif iscellstr(uni_color) %#ok<ISCLSTR> %We can't have continuous color for cellstr input
        obj.continuous_color_options.active = false;
    end
end
...
if ~isempty(obj.aes.facecolor)
    facemap=cell(size(obj.aes.facecolor,1),1);
    for i=1:size(obj.aes.facecolor,1)
        Temp=obj.aes.facecolor{i,1};
        Tempmap=get_colormap(max(unique(Temp)),1,obj.color_options);
        facemap{i,1}=Tempmap(Temp,:);
    end
end
    ...
    for ind_row=1:length(uni_row)
        ...
        for ind_column=1:length(uni_column)
            ...
            if obj.layout_options.hold==0
                ...
                %Make axes current
                axes(obj.facet_axes_handles(ind_row,ind_column)); %#ok<LAXES>
            else
                obj.facet_axes_handles(ind_row,ind_column)=gca;
                axes(obj.facet_axes_handles(ind_row,ind_column)); %#ok<LAXES>
            end
            hold on
                            ...
                            for ind_lightness=1:length(uni_lightness)
                                ...
                                for ind_group=1:length(uni_group)
                                    ...
                                    if ~isempty(sel)
                                        ...
                                        if ~isempty(temp_aes.vertices)
                                            draw_data.vertices=temp_aes.vertices(sel);
                                            draw_data.faces=temp_aes.faces(sel);
                                        end

                                        if ~isempty(temp_aes.u)
                                            draw_data.u=temp_aes.u(sel);
                                        end

                                        if ~isempty(temp_aes.v)
                                            draw_data.v=temp_aes.v(sel);
                                        end

                                        if ~isempty(temp_aes.w)
                                            draw_data.w=temp_aes.w(sel);
                                        end
                                        ...
                                        if ~isempty(temp_aes.pointdata)
                                            draw_data.pointdata=temp_aes.pointdata;
                                        else
                                            draw_data.pointdata=[];
                                        end
     ...
     obj.facet_axes_handles(1,1)=init(1,1,1,...
         obj.layout_options.gap,...
         obj.layout_options.margin_height,...
         obj.layout_options.margin_width,...
         obj.aes.init-1,'Parent',obj.parent,'view',[-37.5 30]);
end
%% draw() Title
if ~isempty(obj.title)
    if obj.updater.first_draw
        ...
        obj.title_text_handle=text(0,0,obj.title,...
            'FontWeight','bold',...
            'Interpreter',obj.text_options.titleinterpreter,...
            'FontName',obj.text_options.font,...
...
for ind_row=1:length(uni_row) %Loop over rows
    ...
    for ind_column=1:length(uni_column) %Loop over columns
        ...
        if obj.axe_options.grid==1
            set(ca,'XGrid','on','YGrid','on','ZGrid','on');
        end

        if obj.axe_options.rad==1
            set(ca,'ThetaAxisUnits','radians');
        end

        if obj.multi.radar
            set(ca,'ThetaZeroLocation',obj.axe_options.zerolocation);
        end

        if obj.axe_options.equal==1
            set(ca,'DataAspectRatio',[1 1 1]);
        elseif obj.axe_options.equal==2
...
if obj.result_ind>1
    for rf=1:length(result_fields)
        ...
        if ~iscellstr(obj.results.(result_fields{rf})) %#ok<ISCLSTR> 
