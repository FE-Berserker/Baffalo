...
p=inputParser;
addParameter(p,'type','ci'); %'95percentile'
addParameter(p,'geom','area');
addParameter(p,'dodge',[]);
addParameter(p,'width',[]);
addParameter(p,'setylim',false);
addParameter(p,'interp','none');
addParameter(p,'interp_in',-1);
addParameter(p,'bin_in',-1);
parse(p,varargin{:});
...
if iscell(draw_data.x) || iscell(draw_data.y) %If input was provided as cell/matrix
    ...
    if params.interp_in>0
        ...
        [~,y]=cellfun(@(x,y)deal(uni_x,interp1(x,y,uni_x,'linear')),draw_data.x,draw_data.y,'UniformOutput',false,'ErrorHandler',@(st,a,b)deal([],[]));
        y=padded_cell2mat(y);
...
try
    switch type
        case 'bootci'
            yci=bootci(nboot,{@(ty) mean(ty,'omitnan') ,y},'alpha',alpha);
        case 'ci'
