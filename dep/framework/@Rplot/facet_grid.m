...
p=inputParser;
addParameter(p,'scale','fixed'); %options 'free' 'free_x' 'free_y' 'independent'
addParameter(p,'space','fixed'); %'free_x','free_y','free'
addParameter(p,'force_ticks',false);
addParameter(p,'column_labels',true);
addParameter(p,'row_labels',true);
parse(p,varargin{:});
