...
out.u=process_mat(out.u);
out.v=process_mat(out.v);
out.w=process_mat(out.w);

if ~isempty(out.pointdata)
    out.pointdata=num2cell(out.pointdata,1); 
    out.pointdata=cellfun(@(c)shiftdim(c),out.pointdata,'uniformOutput',false);
end
% out.pointdata=process_mat(out.pointdata);
...
if ~iscell(out.vertices)&&~isempty(out.vertices)
    out.vertices=num2cell(out.vertices,[1 2]);
end

if ~iscell(out.facecolor)&&~isempty(out.facecolor)
    out.facecolor=num2cell(out.facecolor,[1 2]);
end

if ~iscell(out.faces)&&~isempty(out.faces)
    out.faces=num2cell(out.faces,[1 2]);
end
...
for k=1:length(fields)
    if isempty(aes.(fields{k})) && sum(strcmp(fields{k},...
            {'z' 'ymin' 'ymax' 'label' 'init' 'vertices' 'faces' 'facecolor' 'hold' 'pointdata'}))==0 %If z, ymin, ymax are empty we leave them empty
