...
p=inputParser;
addParameter(p,'position' , 'auto' );
addParameter(p,'legend', true );
addParameter(p,'legend_width', 'auto' );
addParameter(p,'legend_position', 'auto' );
addParameter(p,'title_centering', 'axes');
addParameter(p,'redraw',true);
addParameter(p,'redraw_gap',0.04);
addParameter(p,'margin_height',[0.1 0.2]);
addParameter(p,'margin_width',[0.1 0.2]);
addParameter(p,'gap','auto'); 
% addParameter(p,'gap',[0 0]); 
addParameter(p,'axe',1);
addParameter(p,'hold',0); 
parse(p,varargin{:});
