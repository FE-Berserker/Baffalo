...
addParameter(p,'geom','overlaid_bar'); %line, bar, overlaid_bar, stacked_bar,stairs, point
addParameter(p,'normalization','count');
addParameter(p,'fill',[]); %edge,face,all,transparent
addParameter(p,'width',[]);
addParameter(p,'dodge',[]);
addParameter(p,'aspect',0.3); %Aspect ratio of the inset axis
addParameter(p,'location',[]); %X/Y location in the main axis of the inset axis
parse(p,varargin{:});
...
function obj=my_cornerhist(obj,draw_data,params)
