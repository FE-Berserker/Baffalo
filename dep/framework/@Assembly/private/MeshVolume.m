function [V_hexa,Vc,Vel] = MeshVolume(Node,El)
% Mesh volume
% Author : Xie Yu
...
Vel=zeros(size(El,1),1);
Vc=zeros(size(El,1),3);
...
for i=1:size(El,1)
    ...
    Vel(i,:)=v1 + v2 + v3 + v4 + v5;
    Vc(i,:)=(mean(tet1)*v1+mean(tet2)*v2+mean(tet3)*v3+mean(tet4)*v4+mean(tet5)*v5)/Vel(i,:);
end
