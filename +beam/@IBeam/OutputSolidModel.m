...
if isempty(obj.input.Meshsize)
    obj.input.Meshsize=min(obj.input.t)/3;
end
...
if isempty(obj.input.Stiffner)
    mm=Extrude2Solid(mm,m1,l,ceil(l/obj.input.h*10));
    Vm=PatchCenter(mm);
    Cb=mm.Cb;
    Cb(and(Vm(:,1)==b1/2,Vm(:,2)<0),:)=101;
    Cb(and(Vm(:,2)==-h/2+t1,Vm(:,1)>0),:)=102;
    Cb(and(Vm(:,1)==d/2,Cb==1),:)=103;
    Cb(and(Vm(:,2)==h/2-t2,Vm(:,1)>0),:)=104;
    Cb(and(Vm(:,1)==b2/2,Vm(:,2)>0),:)=105;
    Cb(and(Vm(:,1)==-b1/2,Vm(:,2)<0),:)=106;
    Cb(and(Vm(:,2)==-h/2+t1,Vm(:,1)<0),:)=107;
    Cb(and(Vm(:,1)==-d/2,Cb==1),:)=108;
    Cb(and(Vm(:,2)==h/2-t2,Vm(:,1)<0),:)=109;
    Cb(and(Vm(:,1)==-b2/2,Vm(:,2)>0),:)=110;
    Cb(Vm(:,2)==h/2,:)=111;
    Cb(Vm(:,2)==-h/2,:)=112;
    ...
    m2.Vert=[m1.Vert;m2.Vert;[-m2.Vert(:,1),m2.Vert(:,2)]];
    m2.Face=[m1.Face;m2.Face+Num1;[m2.Face(:,1),m2.Face(:,3),m2.Face(:,2)]+Num1+Num2];
    ...
    for i=1:size(Stiff,1)
        ...
        mm3=Extrude2Solid(mm3,m1,l,ceil(l/obj.input.h*10));
    ...
    Cb=mm.Cb;
    % Beam Cb
    Cb(and(Cb==5,Vm(:,2)<-h/2+t1),:)=101;
    Cb(and(Cb==5,Vm(:,2)>h/2-t2),:)=105;
    Cb(Cb==5,:)=103;
    Cb(and(Cb==4,Vm(:,2)<-h/2+t1),:)=106;
    Cb(and(Cb==4,Vm(:,2)>h/2-t2),:)=110;
    Cb(Cb==4,:)=108;
    Cb(and(and(Cb==7,Vm(:,1)>0),Vm(:,2)==-h/2+t1),:)=102;
    Cb(and(and(Cb==6,Vm(:,1)>0),Vm(:,2)==h/2-t2),:)=104;
    Cb(and(and(Cb==7,Vm(:,1)<0),Vm(:,2)==-h/2+t1),:)=107;
    Cb(and(and(Cb==6,Vm(:,1)<0),Vm(:,2)==h/2-t2),:)=109;
    Cb(and(Cb==6,Vm(:,1)>d/2),:)=111;
    Cb(and(Cb==6,Vm(:,1)<-d/2),:)=112;
    Cb(and(Cb==7,Vm(:,1)>d/2),:)=113;
    Cb(and(Cb==7,Vm(:,1)<-d/2),:)=114;
    Pos2=0;
    Pos3=Stiff(1,1)-Stiff(1,2);
    ...
    for i=1:size(Stiff,1)
        ...
        Pos1=Pos-T/2;
        Pos2=Pos+T/2;
        Cb(and(Vm(:,1)>0,Vm(:,3)==Pos1),:)=1000+4*i-3;
        Cb(and(Vm(:,1)<0,Vm(:,3)==Pos1),:)=1000+4*i-2;
        Cb(and(Vm(:,1)>0,Vm(:,3)==Pos2),:)=1000+4*i-1;
        Cb(and(Vm(:,1)<0,Vm(:,3)==Pos2),:)=1000+4*i;
        if i==size(Stiff,1)
            Pos3=obj.input.l;
        else
