function PlotSyntheticSignal(obj, varargin)
% PlotSyntheticSignal - 绘制合成信号
...
%% 检查输入数据
if isempty(obj.output.t) || isempty(obj.output.s_Synthesis)
    error('请先输入时间(t)和信号(s)数据');
end

if length(obj.output.t) ~= length(obj.output.s_Synthesis)
    error('时间和信号数组长度不一致');
end

%% 解析可选参数
p = inputParser;
addParameter(p, 'Title', '合成信号', @(x) ischar(x) || isstring(x));
