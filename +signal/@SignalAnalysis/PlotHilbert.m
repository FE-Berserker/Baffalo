function PlotHilbert(obj, varargin)
% PlotHilbert - 绘制Hilbert
...
%% 解析可选参数
p = inputParser;
addParameter(p, 'Title', 'Hilbert', @(x) ischar(x) || isstring(x));
...
addParameter(p, 'SavePath', 'Hilbert.png', @(x) ischar(x) || isstring(x));
...
switch opts.Type
    case 0
        t=obj.input.t;
        h = obj.output.Hilbert_s;
    case 1
        t=obj.output.t;
        h=obj.output.Hilbert_s_Transform;
    case 2
        t=obj.output.t;
        h=obj.output.Hilbert_s_Synthesis;
end
