function PlotOriSignal(obj, varargin)
% PlotOriSignal - 绘制原始信号
...
%% 检查输入数据
if isempty(obj.input.t) || isempty(obj.input.s)
    error('请先输入时间(t)和信号(s)数据');
end

if length(obj.input.t) ~= length(obj.input.s)
    error('时间和信号数组长度不一致');
end

%% 解析可选参数
p = inputParser;
addParameter(p, 'Title', '原始信号', @(x) ischar(x) || isstring(x));
...
addParameter(p, 'SavePath', 'ori_signal.png', @(x) ischar(x) || isstring(x));
