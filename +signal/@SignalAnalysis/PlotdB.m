...
%% 解析可选参数
p = inputParser;
addParameter(p, 'Title', 'FFT 分贝', @(x) ischar(x) || isstring(x));
addParameter(p, 'XLabel', '频率[Hz]', @(x) ischar(x) || isstring(x));
addParameter(p, 'YLabel', '分贝dB', @(x) ischar(x) || isstring(x));
...
addParameter(p, 'SavePath', 'FFT_dB.png', @(x) ischar(x) || isstring(x));
...
switch opts.Type
    ...
    case 1
        ...
        y = obj.output.FFT_s_Transform.dB;
    case 2
        %% 检查输入数据
        ...
        y = obj.output.FFT_s_Synthesis.dB;
end
