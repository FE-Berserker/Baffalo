...
%% 解析可选参数
p = inputParser;
addParameter(p, 'Title', 'FFT 幅值', @(x) ischar(x) || isstring(x));
addParameter(p, 'XLabel', '频率[Hz]', @(x) ischar(x) || isstring(x));
addParameter(p, 'YLabel', '幅值P1', @(x) ischar(x) || isstring(x));
...
addParameter(p, 'SavePath', 'FFT.png', @(x) ischar(x) || isstring(x));
...
switch opts.Type
    ...
    case 1
        ...
        y = obj.output.FFT_s_Transform.P1;
    case 2
        %% 检查输入数据
        ...
        y = obj.output.FFT_s_Synthesis.P1;
end
