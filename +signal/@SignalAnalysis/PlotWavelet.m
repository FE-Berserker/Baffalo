function PlotWavelet(obj, varargin)
% PlotWavelet - 绘制小波
...
%% 解析可选参数
p = inputParser;
addParameter(p, 'Title', '小波', @(x) ischar(x) || isstring(x));
...
addParameter(p, 'SavePath', 'wavelet_signal.png', @(x) ischar(x) || isstring(x));
addParameter(p, 'CloseFig', false, @logical);
parse(p, varargin{:});
opts = p.Results;

%% 获取数据
x = obj.output.Wavelet.t;
y = obj.output.Wavelet.w;
