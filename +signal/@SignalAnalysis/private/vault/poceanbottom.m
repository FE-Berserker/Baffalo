function [v,r]=poceanbottom(p,alpha,beta,rho,alphaw,rhow)
% 
% function [v,r]=poceanbottom(p,alpha,beta,rho,alphaw,rhow)
%
% POCEANBOTTOM calculates the ground motion at the ocean bottom 
% for a p-wave incident from below. The
...
%  phone and r*Ao on a radial phone.
      ...
      v=2*xsi.*(1-2*beta^2*p.^2)./(gamma*ro);
      r=2*alpha*p.*((rhow*alphaw*sqrt(1-alpha^2*p.^2)./(rho*alpha*sqrt(1-alphaw^2*p.^2)))+...
           2*eta.*xsi)./ro;