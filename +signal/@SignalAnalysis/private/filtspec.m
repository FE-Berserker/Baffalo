...
%        fmin(2) : crewes.utilities.gaussian width on low end
...
%        fmax(2) : crewes.utilities.gaussian width on high end
...
% design low end crewes.utilities.gaussian
  if fmin(1)>0
...
%HDG removed to be consistent with revised filtf
   ...
   glow=gnot+crewes.utilities.gauss(f,fnotl,fmin(2))';
...
% design high end crewes.utilities.gaussian
 if fmax(1)>0
...
% do phase
  if phase==1
    ...
    fltr=exp(conj(crewes.utilities.hilbm(cmpxspec)));
