...
% 创建几何形状函数 - 使用几何方法添加圆角和尖端，生成整个弹簧的xyz坐标
% Create Geometry - adds fillets and tips, generates xyz coordinates of entire spring

% 此函数使用几何方法在每个弯曲臂的根部添加圆角，
% 为弯曲臂添加圆形尖端，添加圆角连接圆形尖端，
% 创建弹簧的外边缘并将弯曲臂轮廓进行图案化。
% This function uses geometry to add fillets at the root of each flexure,
% add a circular tip to the flexure, add fillets to connect the circular tip,
% create the outer edge of the spring and pattern the flexure profile.
    
    pin_szs = [1 1.5 2 2.5 3 4 5 6 8 10 12 14 16 18 20 25]/2*1e-3; % 标准定位销半径 [m]
    rim_thick = rad_out - rad_root; % 轮缘厚度
    % 如果销半径未指定，根据轮缘厚度估算
    if isempty(pins_rad)
        rough_guess = rim_thick/2; % 粗略估计
    ...
    % 如果销数量未指定，设置为与弯曲臂数量相同
    if isempty(pins_num)
        pins_num = num_flex;
    end
    rad_fs = [rim_thick/2 rim_thick/2]; % [m] 根部圆角半径 [左右]
    rad_tf = ball_rad/2; % [m] 尖端圆角半径
    step_small = step/2; % 小步长
    ...
    for i = 1:2 % 左右两侧循环
        ...
        while bad_fs
            ...
            if abs(atan2(fillet_center(2),fillet_center(1)) - pi/2) <= pi/num_flex
                ...
                rad_fs(i) = rad_fs(i)*0.9; % 减小圆角半径重试
            end
        end
...
%     writematrix((outer_mm),'outer_mm.txt') % flex_uncut
    ...
    %% 绘制弹簧轮廓：如果希望在运行代码后看到图形，请取消注释此部分
