function obj=OutputSolidModel4(obj)
% OutputSolidModel4 - 生成类型4轴支座实体模型
% 类型4特征：长方形底板（带两个平边），中心圆孔，4个固定孔呈方形布置
% Author : Xie Yu
...
% 添加固定孔（4个呈方形布置）
h=Line2D('Hole','Echo',0);     % 创建固定孔线对象
h=AddCircle(h,obj.input.d1/2,a,1,'seg',16);  % 半径为d1/2的圆，16段
...
% 添加4个呈方形布置的固定孔
% F：方形布置的列间距，P：方形布置的行间距
S=AddHole(S,h,'dis',[obj.input.F/2,obj.input.P/2]);    % 右上孔
S=AddHole(S,h,'dis',[-obj.input.F/2,obj.input.P/2]);   % 左上孔
S=AddHole(S,h,'dis',[-obj.input.F/2,-obj.input.P/2]);  % 左下孔
S=AddHole(S,h,'dis',[obj.input.F/2,-obj.input.P/2]);   % 右下孔

%% 生成2D网格
m=Mesh2D('Temp','Echo',0);     % 创建2D网格对象
m=AddSurface(m,S);             % 添加表面到网格
% m=SetSize(m,Meshsize);       % 注释：不设置固定网格尺寸（使用自动划分）
m=Mesh(m);                     % 生成网格
