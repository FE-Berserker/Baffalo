function mm = CreateMesh3D3(obj,mm,r1)
% CreateMesh3D3 - 创建首段实心的多段轴3D网格
% 适用于第一段为实心，后续段有空心部分的轴
% Author: Xie Yu
%
% Inputs:
%   mm  - 网格对象
...
for i=1:size(obj.input.Length,1)-1
    ...
    % 使用线性放样创建侧面
    l=AddElement(l,a1,'transform',[obj.input.Length(i),0,0,0,90,0]);
...
a1=Point2D('Top Verts','Echo',0);
a1=AddPoint(a1,y2,z2);
l=AddElement(l,a1,'transform',[obj.input.Length(end),0,0,0,90,0]);
l=LoftLinear(l,Nmeshes+1,Nmeshes+2,'patchType',patchType,'numSteps',numSteps,'closeLoopOpt',1);
...
for i=size(obj.input.Length,1):-1:1
    ...
    if r2==0  % 如果内径为0，停止（实心部分）
        break
    end

    if i~=size(obj.input.Length,1)
        % 处理内径阶梯面（从当前段到上一段）
        if r2~=obj.input.ID(i+1,1)/2
            x2=repmat(obj.input.Length(i),obj.params.E_Revolve,1);
            y2=cos((0:deg:360-deg)'/180*pi)*r2;
            z2=sin((0:deg:360-deg)'/180*pi)*r2;
            V2=[x2 y2 z2];
    ...
    % 处理内表面（沿x轴反向）
    r2=obj.input.ID(i,2)/2;  % 当前段结束处的内半径
    ...
    a1=Point2D('Top Verts','Echo',0);
    a1=AddPoint(a1,y2,z2);
    l=AddElement(l,a1,'transform',[obj.input.Length(i-1),0,0,0,90,0]);
    l=LoftLinear(l,Nmeshes+1,Nmeshes+2,'patchType',patchType,'numSteps',numSteps,'closeLoopOpt',1);
